...
def fetch_data(team_name:str, team_city:str, week_count_future:int = 1, avoid_home_message:str = ""):
    ...
    games = []
    try:
        r = httpx.get(f"https://api.openligadb.de/getmatchesbyteam/{team_name}/0/{week_count_future}")
        r.raise_for_status()
        games = r.json()
    ...
    try:
        ...
        matchtime = datetime.datetime.strptime(games[0]["matchDateTimeUTC"], "%Y-%m-%dT%H:%M:%S%z")
        timestr = matchtime.strftime("%A, %d.%m.%Y %H:%M")
        dontgo = avoid_home_message if team_name == home else f"Meide Kneipen mit TV in {team_city}"
        location = "Heim" if team_name == home else "Auswaerts"
        out = f"WARNUNG! {timestr}: {home} vs {guest} ({league} — {location}). {dontgo}."
    except IndexError:
